  ...
  try {
    const res = await fetch(
      `${getApiBaseUrl()}/api/artisans?${params.toString()}`,
      {
        ...
        next:    { tags: ['artisans_list'], revalidate: 86400 },
...
export async function fetchArtisanFilters(): Promise<ApiArtisanFilters> {
  ...
  try {
    const res = await fetch(
      ...
      {
        signal:  controller.signal,
        next:    { tags: ['artisans_list', 'artisans_filters'], revalidate: 86400 },
        headers: getApiHeaders(),
      }
    );
...
export async function fetchArtisanDetail(slug: string): Promise<ApiArtisanDetail | null> {
  ...
  try {
    const res = await fetch(
      ...
      { signal: controller.signal, next: { tags: [`artisan_${slug}`], revalidate: 86400 }, headers: getApiHeaders() }
    );
