...
export default function RestaurantsContent({
  ...
  const hasFilters = filters.cities.length > 0 || filters.toques.length > 0 || filters.cuisines.length > 0 || filters.styles.length > 0 || filters.services.length > 0;

  if (initialRestaurants.length === 0 && !hasFilters) {
    return (
      <div className={styles.emptyPage}>
        <p className={styles.emptyPageTitle}>Aucun enregistrement trouvé pour le moment.</p>
        <p className={styles.emptyPageSub}>Revenez bientôt pour découvrir nos restaurants.</p>
      </div>
    );
  }
