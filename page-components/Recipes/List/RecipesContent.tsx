...
import { loadMoreRecipes, searchRecipes, RecipeSearchResult } from '@/lib/actions/recipes';
...
export default function RecipesContent({
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      const results = await searchRecipes(sanitizeSearch(searchQuery));
  ...
  const hasFilters = filters.types.length > 0 || filters.difficulties.length > 0 || filters.chefs.length > 0;

  if (initialRecipes.length === 0 && !hasFilters) {
    return (
      <div className={styles.emptyPage}>
        <p className={styles.emptyPageTitle}>{t('empty.no_records')}</p>
        <p className={styles.emptyPageSub}>{t('empty.subtitle_recipes')}</p>
      </div>
    );
  }
          ...
          {showDropdown && (
            <div className={styles.searchDropdown}>
              <div className={styles.dropdownHeader}>
                <RecipeIcon width={25} height={25} />
                <span className={styles.dropdownLabel}>Recettes</span>
                  ...
                  {searchResults.map((r) => (
                    ...
                    >
                      <div className={styles.dropdownThumb}>
                        ...
                        {r.thumbId && <img src={r.thumbId} alt={r.title} />}
                      </div>
                      <div className={styles.dropdownInfo}>
                        <p className={styles.dropdownTitle}>{r.title}</p>
                        {r.chef && <span className={styles.dropdownTag}>{r.chef}</span>}
                      </div>
                    </Link>
                  ))}
                </div>
